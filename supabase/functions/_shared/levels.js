...
export const PACKS = [
  {
    ...
    hard: {
      ...
      coach: "Pinch it with one hand and move your hand in a circle to turn it.",
  ...
  },
  {
    ...
    hard: {
      ...
      coach: "Build one, grab it with a fist and carry it aside, then build the second to match.",
      points: 250, par: 45,
  ...
  },
  {
    ...
    easy: {
      ...
      coach: "Carry a block by grabbing it with a fist.",
      points: 170, par: 32,
...
/**
 * Run `fn` with Math.random replaced by a seeded stream. Online, every player
 * in a room builds their own board on their own machine, and this is how they
 * all get the same targets: the modes keep calling Math.random, and the room's
 * seed decides what it returns.
 */
export function withSeed(seed, fn) {
  const real = Math.random;
  Math.random = seeded(String(seed));
  try { return fn(); } finally { Math.random = real; }
}

/**
 * An online level race: `levels` themes at one difficulty, or climbing easy to
 * hard when the difficulty is "mixed".
 */
export function raceLadder({ difficulty = "mixed", levels = 5 } = {}, rng = Math.random) {
  const n = Math.max(1, Math.min(levels | 0, PACKS.length));
  if (DIFFICULTIES.includes(difficulty)) {
    return shuffle(PACKS.map((_, i) => i), rng).slice(0, n).sort((a, b) => a - b)
      .map((i) => build(PACKS[i], difficulty));
  }
  const easy = Math.ceil(n / 3), hard = Math.floor(n / 3);
  return tieredDraw({ easy, medium: n - easy - hard, hard }, rng);
}
...
export function checkLevel(level, blocks, planeH) {
  return judge(level, blocks.map((b) => measure(b, planeH)));
}

/**
 * The blocks on a board as plain numbers, in the units the rules are written
 * in. This is what a leaderboard solve sends as its proof: a few numbers per
 * block, no picture.
 */
export function boardOf(blocks, planeH) {
  const r = (v) => +v.toFixed(5);
  return blocks.map((b) => {
    const m = measure(b, planeH);
    return { kind: m.kind, w: r(m.w), h: r(m.h), x: r(m.x), y: r(m.y), angle: r(m.angle) };
  });
}

/**
 * The same verdict for a board that arrived over the wire (see boardOf). The
 * server runs this very file, so a solve it accepts is one the game itself
 * would have accepted.
 */
export function checkBoard(level, board) {
  return judge(level, board.map((b) => {
    const w = +b.w, h = +b.h;
    const deg = +b.angle % 180;
    return { kind: b.kind, w, h, x: +b.x, y: +b.y, angle: deg < 0 ? deg + 180 : deg, span: Math.max(w, h) };
  }));
}

function judge(level, measured) {
  const parts = level.parts;
  const need = parts.length;
  const out = (ok, hint) => ({ ok, hint, have: measured.length, need });

  if (measured.length < need) {
    const missing = need - measured.length;
    return out(false, measured.length === 0
      ? (level.coach ?? level.brief)
      : `${missing} more shape${missing > 1 ? "s" : ""} to go`);
  }
  if (measured.length > need) return out(false, "too many shapes — clear the extras");
